...
describe("склейка классов", () => {
   ...
   * а не порядок написания. Замер показал, что конфликт разрешился бы
   * против нас: `.h-full` стоит ниже ступеней контрола.
   *
   * Радиусов в этом списке больше нет: `rounded-bar` и `rounded-bar-lg`
   * отменены 10.08.2026 вместе со ступенями 2 и 4. Полоска высотой 5
   * с радиусом 2 — это капсула, записанная числом, и она уехала
   * на `rounded-full`, который `tailwind-merge` узнаёт сам.
   */
  test.each([
    ["h-full", "h-ctl-md"],
    ["h-10", "h-ctl-md"],

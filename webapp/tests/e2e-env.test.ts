...
test('composeEnv derives the docker compose port from the resolved test database url', () => {
  ...
  const env = composeEnv({
    DATABASE_URL: 'postgresql://superuser:superpassword@localhost:54330/serch_test?schema=public',
...
test('portFromUrl handles postgres aliases and defaults', () => {
  expect(
    portFromUrl('postgres://superuser:superpassword@localhost/serch_test?schema=public'),
  ).toBe('5432')
  expect(
    portFromUrl('postgresql://superuser:superpassword@localhost/serch_test?schema=public'),
  ).toBe('5432')
...
test('composeEnv defaults a portless postgres URL to the postgres default port', () => {
  ...
  const env = composeEnv({
    DATABASE_URL: 'postgres://superuser:superpassword@localhost/serch_test?schema=public',
    TEST_DATABASE_URL: 'postgres://superuser:superpassword@localhost/serch_test?schema=public',
    POSTGRES_TEST_PORT: '54331',
  })
...
test('composeEnv defaults a portless postgresql URL to the postgres default port', () => {
  ...
  const env = composeEnv({
    DATABASE_URL: 'postgresql://superuser:superpassword@localhost/serch_test?schema=public',
    TEST_DATABASE_URL: 'postgresql://superuser:superpassword@localhost/serch_test?schema=public',
    POSTGRES_TEST_PORT: '54331',
  })
...
test('resolveE2ePorts defaults a portless postgres URL alias to the postgres default port', async () => {
  process.env.TEST_DATABASE_URL = 'postgres://superuser:superpassword@localhost/serch_test?schema=public'
  process.env.POSTGRES_TEST_PORT = '54331'
  ...
  expect(plan.databaseUrl).toBe('postgres://superuser:superpassword@localhost/serch_test?schema=public')
})

test('resolveE2ePorts defaults a portless postgresql URL to the postgres default port', async () => {
  process.env.TEST_DATABASE_URL = 'postgresql://superuser:superpassword@localhost/serch_test?schema=public'
  process.env.POSTGRES_TEST_PORT = '54331'
  process.env.E2E_BACKEND_PORT = '50001'
  process.env.E2E_WEB_PORT = '55001'
  ...
  expect(plan.postgresTestPort).toBe(5432)
  expect(plan.databaseUrl).toBe('postgresql://superuser:superpassword@localhost/serch_test?schema=public')
})
...
test('applyE2ePortEnv overwrites a stale postgres test port with the planned port', () => {
  const plan: PortPlan = {
    ...
    databaseUrl: 'postgresql://superuser:superpassword@localhost:54330/serch_test?schema=public',

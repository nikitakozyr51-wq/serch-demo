...
function ObjectBackBar({
  ...
  return (
    <header
      ...
      className="px-safe-screen flex h-header w-full shrink-0 items-center gap-2 border-b border-line-2 bg-surface"
    >
      ...
      <StickyActionBar>
        <div className="min-w-0 flex-1">
          ...
          */}
          ...
          >
            {/*
              «Раскрыть · 199 ₽», а не «Раскрыть контакт · 199 ₽» — правка
              `PsswU` от 10.08.2026. На 390 длинная подпись прижимала кнопку
              к краю липкой панели, где рядом стоит «Статус». Слово «контакт»
              при этом ничего не добавляет: раскрывают в этом продукте только
              контакт, другого раскрытия нет.

              На компьютере подпись осталась полной — там место есть,
              и кадры `КАБИНЕТ · Карточка объекта` по-прежнему рисуют
              «Раскрыть контакт · 199 ₽». Это не разнобой платформ,
              а разная ширина: короткая подпись заведена там, где длинная
              не помещается.
            */}
            Раскрыть · 199 ₽
          </Button>
 ...
 *
 * **Красной кнопки в покое на этом экране нет ни одной, и это по закону.**
 * Кнопка в строке похожих тёмная, а краснеет она только под курсором и только
 * тогда, когда нажатие правда спишет 199 ₽ (вариант `rowMoney`): красное
 * в покое закреплено за экранами с ОДНИМ действием, а здесь список. На телефоне
 * курсора нет вовсе, поэтому платное от бесплатного тут отличается только
 * словом в подписи — «Раскрыть · 199 ₽» против «Открыть · 0 ₽».
 *
 * Липкая панель тоже графитовая, и не потому, что уступила акцент списку:
 * «Позвонить» по уже раскрытому контакту ничего не списывает, а красный
 * в продукте значит списание, а не важность.
 *
 * Прежняя запись на этом месте объявляла обратное — «кнопка графитовая, потому
 * что красная одна на экран, и она в липкой панели». Неверны были все три
 * утверждения сразу: кнопка строки краснеет под курсором, красной в покое
 * на экране нет, а панель идёт `primary`. Запись осталась от промежуточной
 * редакции закона цвета, которую владелец отклонил 10.08.2026, и вводила
 * в заблуждение ровно того, кто придёт править этот экран следующим.
 */
export function MobileObjectSimilarPage() {
  ...
  /**
   * Раскрытые контакты берутся из сеанса, а не вписаны в `SIMILAR_CARDS`.
   *
   * У тизера `charges` стоял константой `true`, а подпись всегда обещала
   * «Раскрыть · 199 ₽». После первого раскрытия обещание становилось ложью:
   * второе нажатие уходит в ветку «Контакт уже раскрыт, второй раз
   * не списываем» и денег не трогает — а строка по-прежнему говорила
   * про 199 ₽ и считалась платной.
   *
   * Цветом на телефоне это не видно и видно быть не должно: наведения тут нет,
   * а красное в покое владелец отклонил. Но свойство обязано говорить правду
   * и без цвета: оно уезжает в разметку атрибутом `data-charges` — по нему
   * ищут это место, когда разбираются, почему агент потратил 199 ₽, — и по нему
   * же заработает закон цвета, если экран откроют мышью со стенда.
   *
   * Оба соседа считают так же: выдача (`mobile-search-screen`) и полный список
   * похожих ниже в этом файле.
   */
  const disclosed = session?.disclosed ?? []
        ...
        >
          ...
          <div className="flex w-full flex-col overflow-hidden rounded-2xl">
            {SIMILAR_CARDS.map((card) => {
              // Контакт этого похожего объекта агентство уже покупало.
              const paid = disclosed.includes(card.address)
              ...
              return (
                <SimilarRow
                  ...
                  meta={card.meta}
                  why={card.why}
                  // Подпись — единственное, чем платное отличается от бесплатного
                  // на телефоне: цвета для этого нет и не будет. Значит подпись
                  // обязана меняться, иначе строка берёт 199 ₽ за то, что уже
                  // оплачено. Слова те же, что в выдаче и в полном списке
                  // похожих: три экрана про одни и те же деньги не могут
                  // называть их по-разному.
                  actionLabel={paid ? "Открыть · 0 ₽" : card.action}
                  charges={!paid}
                  // Похожий объект открывается, а не только покупается. Открытие
                  // ничего не стоит и обязано быть первым действием строки.
                  //
                  // У оплаченного открывается КАРТОЧКА С НОМЕРОМ, а не экран
                  // продажи: `/m/object/before` про уже раскрытый контакт не
                  // знает и снова предлагает купить. Пока сюда вело жёстко
                  // на `before`, из тизера не было ни одного пути к номеру,
                  // за который агентство уже заплатило.
                  onOpen={() =>
                    void navigate({
                  ...
                  //
                  // У оплаченного списывать нечего, и сообщение «уже раскрыт»
                  // человеку ничего не даёт: он жмёт «Открыть · 0 ₽», чтобы
                  // увидеть номер, — туда и ведём.
                  onAction={() => {
                    if (paid) {
                      void navigate({ to: "/m/object", search: { at: card.address } })
                      return
                    }
                    tellDisclosure(actions.disclose(card.address), navigate)
                  }}
                />
              )
            })}
  ...
  return (
    <PhoneFrame slot="mobile-screen" surface="none">
      <PhoneSheet label="Один объект, четыре объявления">
        ...
        <div className="flex w-full flex-col gap-2">
          {/*
            Красная: это единственное действие экрана, а не строка списка.

            Стояла графитовой с доводом «здесь главное — разбор цены, акцент
            оставлен карточке объекта». Довод отменён замером 10.08.2026:
            «Дубли объекта, откуда цена» нарисованы красной кнопкой
            на обеих платформах — `h2vuK1` на компьютере и `EdWNY/U5Tpd`
            на телефоне, оба `#d92d2c`.

            И это согласуется с законом, а не спорит с ним. Красное в покое
            закреплено не за важностью экрана, а за его УСТРОЙСТВОМ: там,
            где действие одно, красный никого не утомляет — он один в кадре.
            Тёмное в покое оставлено спискам, где кнопок полсотни.
          */}
          <Button
            variant="money"
            size="lg"

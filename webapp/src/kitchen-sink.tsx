  ...
  return (
    <div className="flex min-h-svh w-full flex-col gap-12 bg-bg px-30 py-16">
      ...
      <section className="flex w-full flex-col">
        ...
        {/*
          Этот ряд доски показывает НЕ `primary`, а платную кнопку строки.

          ═══════════════════════════════════════════════════════════════════

          Подпись ряда в файле по-прежнему читается «Primary 48 · заливка fg»,
          но содержимое ячеек — слово «Раскрыть», а наведение перекрашено
          10.08.2026 из `#333029` в `#d92d2c` (узел `CMLdX`). То есть ряд
          демонстрирует принятую редакцию закона цвета: платная кнопка
          в СТРОКЕ тёмная в покое и красная под курсором.

          Поэтому здесь стоит `rowMoney`, а не `primary`. Красить `primary`
          красным на наведении нельзя ни в коем случае: им набраны «Сохранить»,
          «Отправить», «Позвонить» — действия, которые денег не стоят,
          и красный на них означал бы списание, которого не будет.

          **Два расхождения с файлом названы, а не подогнаны.**

          Первое: подпись ряда осталась от прежнего смысла. Второе: ряд
          нарисован ступенью 48, а платная кнопка строки в продукте идёт 32 —
          на 48 «Раскрыть» существует только на карточке объекта, и там она
          красная В ПОКОЕ, а не от наведения. Ступень взята из ряда как есть,
          потому что доска проверяет ЦВЕТА состояний, а не размещение.

          Следствие, которое надо знать: собственных пяти состояний
          у `primary` на доске больше нет, и его наведение сверкой
          не покрыто. Это потеря покрытия, а не решение, — вернуть ряд
          для `primary` должен дизайн-чат.
        */}
        <Row
          id="row-money-48"
          name="Раскрыть · 199 ₽"
          spec="48 · r-pill · 16 · покой fg, наведение accent-deep"
          cells={[
            <Button variant="rowMoney" size="lg">
              Раскрыть
            </Button>,
            <Button variant="rowMoney" size="lg" demo="hover">
              Раскрыть
            </Button>,
            <Button variant="rowMoney" size="lg" demo="focus">
              Раскрыть
            </Button>,
            <Button variant="rowMoney" size="lg" demo="press">
              Раскрыть
            </Button>,
            <Button variant="rowMoney" size="lg" disabled>
              Раскрыть
            </Button>,
          ]}
        />
      ...
      <section className="flex w-full flex-col gap-6">
        ...
        {/*
          ...
          908 — номинальная ширина компонента `jsW77`, и она была настоящей,
          пока слева от выдачи стояла колонка фильтров 260. С 9 августа
          колонки нет, выдача выросла до 1200, и продукт инстансирует строку
          на 1168 в просторном и 1176 в плотном. Держать полигон на 908
          значит проверять ширину, которой нет ни на одном экране: адресу
          там остаётся 84 пикселя при нужных 123, и строка ломается — но
          ломается только на полигоне.

          Расхождение осталось и в самом файле: компонент `jsW77` по-прежнему
          908, а все четыре выдачи инстансируют его шире. Названо дизайн-чату,
          чтобы компонент подтянули к экранам.
        */}

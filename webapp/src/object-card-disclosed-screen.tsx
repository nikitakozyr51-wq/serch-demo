...
function SimilarRow({
  ...
  paid: boolean
  /**
   * Открыть карточку похожего объекта.
   *
   * До этой правки в строке работала только кнопка раскрытия — то есть
   * единственным доступным действием было заплатить. Посмотреть похожий
   * объект, прежде чем платить за его контакт, было нельзя. Открытие
   * не стоит ничего и обязано быть первым.
   */
  onOpen: () => void
  onAction: () => void
}) {
  return (
    <div
      ...
      // а не таблица выдачи.
      className="row-tap flex w-full cursor-pointer items-center gap-5 border-t border-line-1 py-5 first:border-t-0"
    >
      {/*
        Пара кадров «этот и похожий» — 48 × 48 каждый, радиус `r-control` 12,
        зазор 6, колонка 168. Замер `GvT0A/tUfwy` от 10.08.2026.

        Стояли 84 × 56 три к двум. Квадрат пришёл вместе со строкой списка:
        сверка свела все кадры В СПИСКАХ к одной пропорции на обеих
        платформах, и блок сравнения — тот же список. Крупные кадры карточки
        и галерея не тронуты: там снимок показывают, а не перечисляют.

        Подпись «КУХНЯ И КУХНЯ» здесь стоит НАД кадрами, а на телефоне под
        ними — и это не разнобой, а разная раскладка. На компьютере строка
        горизонтальная и колонки выровнены по центру: подпись сверху служит
        колонке заголовком. На телефоне колонки выровнены по верху, и подпись
        над кадрами опускала бы их на 22 px ниже адреса справа.
      */}
      <div className="flex w-42 shrink-0 flex-col gap-1.5">
        ...
        <div className="flex w-full gap-1.5">
          {[0, 1].map((index) => (
            <div key={index} className="size-12 shrink-0 overflow-hidden rounded-xl bg-warm">
              <ListingPhoto
      ...
      */}
      ...
      {/*
        ...
        Теперь это настоящая кнопка продукта — с наведением, фокусом
        и нажатием.

        **Тёмная в покое, красная под курсором — и только платная.**

        Это строка СПИСКА, а не единственное действие экрана. Промежуточная
        редакция красила её красной в покое, и владелец отклонил именно
        на этом экране: до семи красных блоков в одном кадре. Красное
        в покое осталось за карточкой объекта, листом массового раскрытия
        и диалогом подтверждения — там кнопка одна, и заливать ей нечего.

        Различать платное и бесплатное всё равно нужно: рядом стоят
        «Раскрыть · 199 ₽» и «Открыть · 0 ₽». Различие приходит с курсором,
        то есть ровно в момент прицеливания, и только на той кнопке,
        которая стоит денег.
      */}
      {/* Всплытие гасится: «Раскрыть · 199 ₽» и «открыть карточку» — разные
          намерения, и одно нажатие не может значить оба. */}
      <Button
        variant={paid ? "row" : "rowMoney"}
        size="sm"
        onClick={(event) => {
          ...
          {(allSimilar ? similar : similar.slice(0, 3)).map((item) => {
            ...
            return (
              <SimilarRow
                ...
                item={item}
                paid={paid}
                // Карточка похожего открывается у любого: просмотр бесплатен,
                // а до раскрытия и после это два разных экрана.
                onOpen={() =>
                  void navigate({
                    to: paid ? "/object/disclosed" : "/object",
                    search: { at: item.address },
                  })
                }
                onAction={() => {
                  /*
                    ...
                    **Адрес обязателен.** Без него маршрут отдаёт пустой поиск,
                    а карточка берёт запасной адрес «Ленская ул., 10» — то есть
                    агентство платит 199 ₽ за один контакт и видит на экране
                    телефон другого собственника. Промах наступал ровно после
                    списания: подпись становится «Открыть · 0 ₽» именно тогда,
                    когда деньги уже ушли.

                    Заметно это было тем, что нажатие по СТРОКЕ (`onOpen` выше)
                    уводило на нужный объект, а нажатие по её собственной
                    кнопке — на чужой. Разница в восемь пикселей вправо.
                  */
                  if (paid) {
                    void navigate({
                      to: "/object/disclosed",

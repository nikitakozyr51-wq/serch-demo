...
function stateOf(item: Listing, index: number): Pick<SearchRow, "status" | "takenBy" | "action"> {
  ...
  // Каждый двадцать третий объект — стоп-лист. Число простое, чтобы отметки
  // не выстраивались в узор рядом с другими признаками.
  if (index % 23 === 0) {
    return { status: "stop-list", action: { kind: "blocked", label: "Просил не звонить" } }
  }

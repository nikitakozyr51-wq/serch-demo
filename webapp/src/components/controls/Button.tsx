 ...
 *
 * Лестница закрыта — высота, радиус и кегль подписи связаны жёстко:
 *
 *   sm  32 → 28   r-8    14 → 13   вторичная, кнопка в строке
 *   md  40 → 36   r-10   14 → 13   кнопка формы
 *   lg  48 → 44   r-12   16        главная; у primary и money радиус pill
 *
 * Высоту переключает плотность через переменные `--h-*`, кегль — вариант
 * `compact` внутри `typography.tsx`. Размеров шрифта здесь нет намеренно:
 * ESLint запрещает типографические утилиты вне компонента Typography.
 */
const buttonVariants = cva(
  ...
  ],
  {
    variants: {
      ...
      size: {
        sm: "h-ctl-sm rounded-md px-4",
        md: "h-ctl-md rounded-lg px-4",
        lg: "h-ctl-lg rounded-xl px-7",
      },
    },
    compoundVariants: [
      // Pill закреплён за главным действием и за списанием. Везде остальном он
      // превращает плотный список в мешок леденцов.
      { variant: "primary", size: "lg", class: "rounded-full" },
      { variant: "money", size: "lg", class: "rounded-full" },
      { variant: "pending", size: "lg", class: "rounded-full" },
    ],
    defaultVariants: {
      variant: "secondary",
      size: "sm",
...
function Button({
  ...
  return (
    <Comp
      ...
      // Через `cn`, а не напрямую: у primary и money радиус pill задан
      // поверх радиуса размера, и без разрешения конфликта побеждает не тот,
      // что написан последним, а тот, что стоит позже в собранном CSS.
      // Кнопка выходила с радиусом 12 вместо 999 — поймано замером.

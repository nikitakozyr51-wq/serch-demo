 ...
 *
 * ═══════════════════════════════════════════════════════════════════════════
 * ПОЧЕМУ ЭКРАН ПОЯВИЛСЯ ЗДЕСЬ, А НЕ СНЯТ С КАДРА
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * Кадр 404 в файле есть, но он сайтовый (`mEcGL`, телефонный `gKAga`):
 * кегли 26 · 17 · 15 — лестница сайта, а не кабинета, и говорит он с чужим
 * человеком, который искал страницу отказа от звонков. Сайт его уже собрал.
 *
 * У кабинета своего кадра 404 нет, а неверный адрес в кабинете открывался.
 * И открывал он чужую заготовку из шаблона проекта: «Page not found»,
 * «Return to workspace» — по-английски, в русском продукте, да ещё через
 * проверку сеанса на сервере, которой у кабинета нет. То есть человек,
 * ошибившийся в адресе, видел не продукт, а его строительные леса.
 *
 * Поэтому экран собран из УЖЕ СУЩЕСТВУЮЩЕГО языка кабинета: тот же каркас
 * `AuthShell`, что у «Доступа закрыт» — ближайшего готового экрана-тупика.
 * Ни одного нового значения не заведено; выдумывать геометрию там, где
 * кадра нет, нельзя.
 *
 * **Тупик с выходом, а не сообщение об ошибке.** Куда идти — зависит от
 * того, вошёл человек или нет: вошедшего ведём в его поиски, чужого —
 * на вход. Кнопка, ведущая в кабинет того, кто в него не заходил,
 * упёрлась бы в дверь и вернула бы его сюда же.
 */
export function NotFoundScreen() {
  ...
  return (
    <AuthShell
      ...
      subtitle="Возможно, ссылка устарела или в адресе опечатка. Если вы шли по ссылке на подборку, попросите агента прислать её заново: ссылки на подборки можно отключать."
      stepsLabel="КУДА МОЖНО ПОЙТИ"
      showPricing={false}
      steps={[
        {
          title: session ? "Сохранённые поиски" : "Вход в кабинет",
          note: session
            ? "главный экран кабинета: что нового по вашим условиям"
            : "почта и пароль агентства",
        },
        {
          title: "serch.ru/stop",
          note: "исключить свой номер из сервиса, без регистрации",
        },
        {
          title: "serch.ru/data",
          note: "узнать, откуда у нас номер: площадка, объявление, дата",
        },
      ]}
      protection="Если нужна помощь человека, напишите на hello@serch.ru."
    >
      <div className="flex items-center gap-5">
        <Button
          variant="primary"
          size="lg"
          onClick={() => {
            if (session) void navigate({ to: "/searches" })
            else void navigate({ to: "/login", search: { returnTo: undefined } })
          }}
        >
          {session ? "К моим поискам" : "На страницу входа"}
        </Button>
        <AuthAside
          question="Искали страницу отказа от звонков?"
          action="Она живёт по адресу serch.ru/stop"
        />
      </div>
    </AuthShell>
  )

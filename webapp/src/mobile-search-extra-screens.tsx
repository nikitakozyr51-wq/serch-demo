    ...
    <PhoneStand surface="none">
      <PhoneSheet label="Раскрыть 9 контактов?">
        <div className="flex w-full flex-col gap-2">
          ...
          {/*
            Предупреждение про журнал доступа — правка `TYMiJ` от 10.08.2026.

            Массовое раскрытие девяти контактов оставляет девять записей
            с именем, временем и IP. На компьютере это сказано в окне
            подтверждения (`dialog-screens.tsx`), а на телефоне человек
            подтверждал списание, не зная о следе. Одно и то же действие
            обязано предупреждать об одном и том же с обеих платформ.
          */}
          <Typography variant="metaText" tone="dense">
            Каждое раскрытие попадёт в журнал доступа с вашим именем, временем
            и IP.
          </Typography>
        </div>

        <div className="flex w-full flex-col gap-2">
          {/* Красная: нажатие списывает деньги. Закон цвета действия
              один на обе платформы, и «Раскрыть 9 · 1 791 ₽» под него
              попадает так же, как «Раскрыть · 199 ₽» в строке. */}
          <Button
            variant="money"
            size="lg"
            block

...
export function BalanceTopUpPage() {
   ...
   *
   * Сумма запоминается в момент нажатия, а не читается из поля при показе
   * окна: поле остаётся живым под затемнением, и человек, крутнувший колесо
   * мыши, увидел бы в подтверждении не ту сумму, которую только что внёс.
   */
  const [credited, setCredited] = useState<number | null>(null)
  // Окно уходит за 120 мс, а не пропадает за кадр: исчезновение видно лучше
  // появления — человек уже смотрит туда, потому что сам себя туда привёл.
  const done = useExitValue(credited)
      ...
      <CabinetPage>
        ...
        <div className="flex w-full items-start gap-8">
          ...
          <aside className="flex w-95 shrink-0 flex-col gap-4 rounded-2xl border border-line-2 bg-surface p-5">
            ...
            {/* Пополнение обязано доходить: деньги приходят на общий счёт
                агентства, и счётчик в шапке досчитывает их на глазах.
                Без этого демонстрация показывает форму вместо системы.

                Само нажатие теперь ещё и отвечает: до окна подтверждения
                единственным следом двадцати тысяч было изменившееся число
                в шапке, которое человек, смотрящий на кнопку, не видит. */}
            <Button
              variant="primary"
              size="lg"
              block
              ...
              onClick={() => {
                topUp(amount, method === "card" ? "карта" : "счёт на юрлицо")
                setCredited(amount)
              }}
            >
              Скачать счёт
        ...
        {/* СОСТОЯНИЕ · Баланс пополнен (`ZmKkW`). Окно, а не сообщение:
            деньги оставляют постоянный след, и подтверждение обязано
            дождаться, пока его прочтут. */}
        {done.mounted && done.value !== null ? (
          <TopUpDoneDialog
            amount={done.value}
            leaving={done.leaving}

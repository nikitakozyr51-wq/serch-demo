...
export type Workspace = {
   ...
   * ═══════════════════════════════════════════════════════════════════════
   *
   * Деньги живут здесь, а не в сеансе, и это не перекладывание поля.
   * Счёт принадлежит АГЕНТСТВУ, а не человеку: шапка так и подписана —
   * «Баланс агентства». Пока он лежал в сеансе, у каждого сотрудника был
   * свой отдельный кошелёк, и приглашённый агент, войдя в контору с
   * восемью тысячами на счету, видел ноль и не мог раскрыть ни одного
   * контакта. Одно агентство с двумя разными остатками — это не продукт.
   *
   * Сеанс продолжает показывать те же два числа, но как ЗЕРКАЛО: экраны
   * читают их по-прежнему из сеанса, а правда одна и лежит здесь.
   */
  balance: number
  trial: number
}
...
/**
 * Ключ хранилища привязан к почте ВЛАДЕЛЬЦА агентства, а не к почте
 * вошедшего.
 *
 ...
 * и приглашённый агент открывает пространство по почте того, кто его позвал.
 */
...
export function openWorkspace(email: string) {
  ...
  void loadWorkspace()
    .then((remoteWorkspace) => {
      ...
      /**
       * Деньги сервер отдаёт не журналами, а вместе с личностью, поэтому
       * приехавшее пространство их не знает и держит нули. Затереть ими
       * счёт агентства значило бы обнулить его на глазах у человека —
       * сохраняем то, что уже посчитано.
       */
      write({ ...remoteWorkspace, balance: current.balance, trial: current.trial })
    })
    .catch((error: unknown) => {
...
export function initWorkspace(
  owner: Omit<Person, "id" | "addedAt" | "role" | "limit">,
  money: { balance: number; trial: number },
) {
  const at = stamp()
  write({
    ...EMPTY,
...
/**
 * Записать счёт агентства.
 *
 * Единственный вход: списание, пополнение и возврат зовут его, а не правят
 * поле. Второго места, где меняются деньги, быть не должно.
 */
export function setMoney(money: { balance: number; trial: number }) {
  write({ ...current, ...money })
}

/**
 * Дописать человека в агентство, если его там ещё нет.
 *
 * Ключ — почта: имя человек меняет, почта остаётся тем, по чему он входит.
 * Повторный приём того же приглашения не должен плодить двойников.
 *
 * ═══════════════════════════════════════════════════════════════════════════
 * ЗОВЁТСЯ НЕ ТОЛЬКО ПО ПРИГЛАШЕНИЮ, НО И ПРИ КАЖДОМ ВХОДЕ
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * Состав агентства заводится один раз — когда агентство создают. А войти
 * в него можно с другого компьютера, из другого браузера или после чистки
 * хранилища: сеанс есть, состава на этой машине нет. Список людей оставался
 * пустым, и продукт отвечал на вопрос «сколько нас» двумя числами
 * на соседних экранах — «Эффективность» говорила «0 сотрудников»,
 * «Сотрудники» показывали одного. Второе число было верным, но добывалось
 * не из состава: экран подставлял вошедшего сам. Это чинило один экран
 * и оставляло дыру в выгрузке, в назначении объектов и в лимитах.
 *
 * У руководителя опознаваемый идентификатор `owner`, а не считанный: по нему
 * его находит передача роли, и он обязан быть одним и тем же на всех машинах.
 */
export function joinWorkspace(person: Omit<Person, "id" | "addedAt">) {
  if (current.people.some((item) => item.email === person.email)) return
  write({
    ...current,
    people: [
      ...current.people,
      {
        ...person,
        id: person.role === "owner" ? "owner" : nextId("p"),
        addedAt: stamp(),
      },
    ],
  })

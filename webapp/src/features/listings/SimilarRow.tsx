  ...
  return (
    <div
      ...
      className={cn(
        ...
        //
        // Первой строке линия не положена — спека, раздел 2.4: линия сверху
        // каждой строки, КРОМЕ ПЕРВОЙ. Пока строки были двумя кусками
        // разметки, обе копии берегли первую сами: мобильный список ставил
        // линию только при `index > 0`, а десктопный двойник в
        // `object-card-disclosed-screen.tsx` гасит её через `first:border-t-0`
        // и сейчас. Сведение форм в один компонент это условие потеряло,
        // и линия поехала на верхнюю кромку контейнера — а он
        // `overflow-hidden rounded-2xl` с белой заливкой, так что черта
        // легла ровно под заголовком блока и читалась лишней.
        //
        // Гасится CSS-ом, а не пропом, по образцу двойника: порядок строк
        // знает родитель, и браузер спрашивает о нём сам. Проп на том же
        // месте дал бы вызывающему возможность соврать — оставить линию
        // первой строке или снять её у средней, — то есть завёл бы способ
        // сломать правило там, где сейчас сломать его нечем.
        "shadow-[inset_0_1px_0_var(--line-1)] first:shadow-none",
        "row-tap",
        wide ? "px-4 py-3" : "p-3",
      )}
    >
      ...
      <div className="flex w-full items-center gap-2">
        ...
        <button
          ...
          className={cn(
            ...
            /*
              Тот же закон, что в выдаче: в СТРОКЕ кнопка тёмная, красной
              становится под курсором и только платная.

              Промежуточная редакция красила её красной в покое — и именно
              этот экран владелец назвал доводом против: на «Похожих»
              доходило до семи красных блоков в одном кадре.

              Компонент телефонный, а на телефоне наведения нет, поэтому
              `mouse:` тут почти всегда молчит. Правило всё равно записано:
              экран открывают и со стенда на компьютере, и одинаковая
              строка на двух платформах обязана вести себя одинаково там,
              где условия совпали.
            */
            charges
              ? "bg-fg text-surface mouse:hover:bg-accent-deep active:bg-fg-press"

...
function FilterBar({
  ...
  return (
    <div
      ...
      /*
        Поле справа 8 — компенсация подложки «Сбросить».

        У кнопки сброса поле 8 и отрицательное поле −8: так подпись стоит
        на своём месте по кадру, а подложка появляется вокруг неё под пальцем.
        Без компенсации подложка вылезает за правый край полосы на эти самые
        8 пикселей — проверка переполнения ловит это как «нужно 1176, есть
        1168», и ловит правильно.

        Восемь пикселей подписи при этом уходят внутрь: в кадре «Сбросить»
        стоит вплотную к краю полосы, здесь — на 8 левее. Названо, а не
        спрятано; выбор между этим и вылезающей подложкой сделан в пользу
        того, что не вылезает.

        Линии снизу нет: в кадре `ghwPj` полоска идёт без обводки, её от
        соседнего блока отделяет зазор 12.
      */
      className="flex h-11 w-full shrink-0 items-center gap-3 pr-2"
    >
      <button
        type="button"

  ...
  return (
    <>
      {/*
        ...
        ═══════════════════════════════════════════════════════════════════
        ШАГ БЕЗ ПОДСВЕТКИ НЕ ЗАТЕМНЯЕТ НИЧЕГО
        ═══════════════════════════════════════════════════════════════════

        Из двадцати шагов подсвечивают что-то девять. Остальные одиннадцать
        говорят про экран целиком — «Прозвон подряд», «Баланс агентства»,
        «Журнал доступа», — и показывать их было нечем: код заливал
        затемнением весь экран и оставлял поверх одну выноску. Владелец
        увидел ровно это: «просто серый экран становится».

        Так и нарисовано в файле, и это проверено замером: у кадров
        `hWDUt` и `A7I9fQ` есть слой «Затемнение» 1440 × 1024 `#1e1e1e59`
        и поверх него «Подсвечено»; у кадров `Tm0Sj`, `zCsNu`, `mj2DO`
        слоя «Затемнение» НЕТ ВОВСЕ — только выноска над живым экраном.

        Логика очевидна, когда её произнесёшь: затемнение существует, чтобы
        отделить одно от остального. Когда речь про весь экран, отделять
        не от чего, и затемнение остаётся серой пеленой без смысла — оно
        не помогает смотреть, оно мешает.

        ═══════════════════════════════════════════════════════════════════
        НО САМ СЛОЙ ОСТАЁТСЯ ВСЕГДА, И ЭТО НЕ ФОРМАЛЬНОСТЬ
        ═══════════════════════════════════════════════════════════════════

        Первая попытка снимала вместе с затемнением весь узел — и снимала
        вместе с ним то, ради чего он вообще нужен помимо цвета: он ловит
        нажатия. Без него на одиннадцати шагах из двадцати под окном
        обучения оставался ЖИВОЙ продукт, и красная «Раскрыть · 199 ₽»
        списывала деньги, пока человеку рассказывали, что она делает.
        Обучение, которое тратит деньги агентства, — это не обучение.

        Заодно пропадал второй способ выйти: нажатием мимо окна.

        Поэтому слой стоит на месте всегда, а меняется у него только
        заливка. Прозрачный он или тёмный — вопрос смысла; ловит он
        нажатия или нет — вопрос безопасности, и ответ на него один.
      */}
      {/* Заливки нет ни в одном случае: когда подсветка есть, затемнение
          рисует тень выреза ниже; когда её нет — затемнения не должно быть
          вовсе. Прозрачный слой нажатия ловит ровно так же, как тёмный. */}
      <div

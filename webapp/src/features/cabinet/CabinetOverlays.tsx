...
function CabinetOverlays() {
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      /*
        Escape проверяется ДО того, как обработчик уступает набору текста.

        ═══════════════════════════════════════════════════════════════════════

        Правило «пока человек печатает, буквенные клавиши ему и достаются»
        верное: иначе «?» посреди заметки открывал бы справку. Но Escape
        буквой не является — он и придуман как выход из того, что открыто,
        и обязан работать особенно тогда, когда фокус внутри окна.

        Поймано проверкой движения. Когда в палитру добавили поле ввода
        с автофокусом, Escape начал попадать в это поле, обработчик уступал
        ему по общему правилу — и палитра перестала закрываться клавишей
        вообще. Мышью мимо окна она закрывалась, и глазами дефект не виден:
        окно закрывается, просто не тем способом, которым обещано.

        Проверка увидела это как «затемнение палитры не играет кадр ухода»:
        уход не начинался, потому что нечему было уходить.
      */

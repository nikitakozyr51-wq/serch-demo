...
import { TopUpRequestSheet } from "@/mobile-money-sheets"
import {
  RefundLimitSheet,
  RefundSentSheet,
  TopUpDoneSheet,
 ...
 *
 * Третье: главная кнопка раздела зависит от роли. Руководитель пополняет,
 * агент запрашивает пополнение листом `PkIhp` — счёт общий, и права на него
 * у двоих разные. Правило пришло не отсюда: оно уже описано в шапке кабинета
 * на компьютере, и телефон его повторяет.
 */
...
/**
 * Главное действие раздела денег — и у агента оно ДРУГОЕ.
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *
 ...
 * руководитель: у агента нет ни доступа к реквизитам, ни права обязывать
 * агентство платить. Правило уже описано в шапке кабинета на компьютере,
 * и телефон его повторяет, а не переизобретает: «Пополнить» вело агента
 * на форму, где он упирался либо в пустоту, либо в отказ.
 *
 * «Запросить пополнение» — то, что агент правда может: сказать руководителю,
 * что деньги кончились. Оно открывает лист `PkIhp` поверх списка, а не уводит
 * с экрана: остаток и история списаний остаются за затемнением, и именно
 * из-за них запрос и отправляют.
 *
...
function TopUpButton() {
  ...
  if (session?.role === "agent") {
    return (
      <>
        <Button variant="primary" size="lg" block {...pressProps(() => setAsking(true))}>
          Запросить пополнение
        </Button>
        {asking ? <TopUpRequestSheet onClose={() => setAsking(false)} /> : null}
      </>
    )
  }
...
export function MobileBalanceRefundsPage() {
  ...
  return (
    ...
    >
      <MoneyBody>
        {/*
          Крупным числом — только рубли. Правка `lmGuS` от 10.08.2026: штуки
          и рубли на одной шкале читаются одной величиной, а это две разные.
          Сколько было заявок — в подписи, где оно и есть подробность.
        */}
        <BalanceHero
          value={groupDigits(returned) + " ₽"}
          caption={`возвращено за 30 дней · ${monthly.length} ${plural(monthly.length, "заявка", "заявки", "заявок")}${share}`}
        />
        {/*
          «Пополнить» на вкладке возвратов больше нет — узел `d175Il`
          выключен 10.08.2026.

          Вкладка отвечает на вопрос «сколько нам вернули за брак», и главное
          действие раздела денег на ней стояло по инерции — оно одинаково
          прилеплено ко всем четырём вкладкам. Но пополнение к возвратам
          отношения не имеет: человек, пришедший смотреть возвраты, пришёл
          проверять качество базы, а не вносить деньги. Кнопка осталась
          на трёх вкладках, где она по делу: остаток, списания, пополнения.
        */}

...
type TodayCard = {
  ...
  blocked?: boolean
  /**
   * Контакт по объекту уже оплачен — от этого зависит, какую карточку открыть.
   *
   * Спрашивается у ЖУРНАЛА АГЕНТСТВА, а не у списка раскрытий своего сеанса.
   * Причина та же, по которой раскрытие спрашивает журнал: «если номер уже
   * открывал коллега, второй раз агентство не платит». Список дня и сам
   * собран из журнала, а личный список у коллеги пуст — по нему строка увела
   * бы на витрину с «Раскрыть · 199 ₽» по контакту, за который агентство
   * уже заплатило.
   */
  paid: boolean
}
...
function buildCards(workspace: Workspace, now: number, mine: string): TodayCard[] {
  ...
  const cardFor = (address: string, _touch: string, due: string, tone: TodayCard["dueTone"]) => {
    ...
    return {
      ...
      paid: isDisclosed(workspace, address),
...
/**
 * Строка дня ОТКРЫВАЕТ КАРТОЧКУ ОБЪЕКТА.
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * Строка была мёртвой. Раньше в каждой строке стоял свой переход на прозвон; его
 * убрали в пользу одной тёмной кнопки внизу экрана — и это верно, у экрана
 * дня одно главное действие, — но нажатие на саму строку при этом не завели.
 * Осталась `<article>` без перехода: выглядит она ровно как строка выдачи
 * и как строка похожих — снимок 48, адрес, цена, отклонение, волосяная линия,
 * — обе те строки нажимаются целиком, а эта не делала ничего.
 *
 * Хуже того, из строки намеренно вырезаны мета, история касаний и причина
 * запрета — с обоснованием «всё это живёт на карточке объекта, куда человек
 * и так зайдёт перед звонком». Зайти было некуда: факты убрали, а дверь к ним
 * не открыли. Теперь дверь есть, и обоснование стало правдой.
 *
 * **Куда ведёт.** `/m/object` — карточка с номером, если контакт агентством
 * оплачен; `/m/object/before` — витрина с раскрытием, если ещё нет. Та же
 * развилка, что в выдаче на телефоне: два списка одного продукта не могут
 * вести в разные места по одному и тому же объекту.
 *
 * Заблокированный объект открывается тоже. Запрет относится к звонку, а не
 * к чтению: карточка как раз и объясняет, почему по этому объекту работать
 * нельзя, — и других мест, где это написано, у телефона нет.
 *
 * **Ссылкой, а не `div` с `onClick`.** Переход известен заранее и ничего
 * не списывает, поэтому строка обязана уметь клавиатуру, Enter, средний
 * щелчок и открытие в новой вкладке. Строка выдачи `MobileListingRow` сделана
 * `div`-ом не от хорошей жизни: внутри неё лежит кнопка «Раскрыть · 199 ₽»,
 * а кнопку в ссылку класть нельзя. Здесь вложенных контролов нет, и форма
 * взята у строки подборки (`mobile-collections-screens.tsx`), которая тоже
 * просто ведёт внутрь.
 *
 * **Отклик на касание — общий `.row-tap`**, тот же, что у строки выдачи
 * и у строки похожих. Расхождение названо вслух: у заблокированной строки
 * класс берёт ветку `[data-blocked]`, написанную для ТЁПЛОЙ строки — в выдаче
 * и на компьютере объект из стоп-листа лежит на `warm`, — а эта строка белая
 * и по файлу такой и остаётся, стоп-лист в ней показан чипом. Следствие:
 * нажатие на строку из стоп-листа гаснет на ступень глубже соседних
 * (`warm-press` вместо `warm-hover`). Оба значения из палитры, ничего
 * не выдумано; чинится либо замером тёплой подложки, либо сужением правила
 * в `index.css`, и то и другое — отдельная задача.
 */
function TodayCardRow({ card }: { card: TodayCard }) {
  return (
    <Link
      to={card.paid ? "/m/object" : "/m/object/before"}
      search={{ at: card.address }}
      data-slot="today-card"
      // Адрес атрибутом — как в строке выдачи: по нему проверка пути находит
      // объект, который человек правда выбрал, а не угадывает его по тексту.
      data-address={card.address}
      data-blocked={card.blocked || undefined}
      className="row-tap flex w-full cursor-pointer items-center gap-3 bg-surface px-3 py-4 shadow-[inset_0_-1px_0_var(--line-1)] outline-none last:shadow-none focus-visible:outline-solid focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-fg"
    >

...
// Ступени 11, 13 и плотная 20 названы числом, а не словом, и прежняя
// регулярка их не ловила: числовая альтернатива требовала суффикса `xl`.
// Значит `text-11` можно было написать в любом файле продукта, и правило
// «типографика только внутри Typography» на него не распространялось.
// Тот же класс промаха, что с tailwind-merge: имя, названное числом,
// выпадает из фильтра по имени.
const projectTextSteps = '11|13|20-tight'
const textSizePattern = new RegExp(
  String.raw`^text-(xs|sm|base|lg|xl|[2-9]xl|[1-9]\d+xl|${projectTextSteps})${lineHeightShorthandPattern}$`
)
const fontPattern =

...
function build(where, label) {
  ...
  const result = spawnSync('bun', ['run', 'build'], {
    ...
    /**
     * ДЕМОНСТРАЦИЯ СОБИРАЕТСЯ БЕЗ БАЗЫ, И ЭТО РЕШЕНИЕ.
     *
     * `webapp/.env.local` разработчика указывает на облачный Supabase.
     * Без явного затирания эти значения запеклись бы в публичную сборку —
     * и любой, кто нашёл адрес, регистрировался бы в настоящей базе
     * агентства. Подтверждение почты выключено, тарифных ограничений нет:
     * это открытая дверь, а не демонстрация.
     *
     * Демонстрация работает из браузера посетителя: каждый заводит своё
     * агентство у себя, никто не видит чужое, и портить нечего. Ровно
     * то, что нужно от витрины.
     *
     * Когда рабочая версия поедет на свой домен, база включится там —
     * отдельной сборкой со своими значениями, а не этой.
     */
    env: {
      ...process.env,
      SERCH_BASE: base,

...
//      freshness instead.
/**
 * Куда сайт ляжет на хостинге.
 *
 * На своём домене корень пустой. На GitHub Pages для проекта корень равен
 * имени репозитория: `nikitakozyr51-wq.github.io/serch-demo/`. Astro сам
 * подставит это во все внутренние ссылки, если сказать ему `base`.
 *
 *     SERCH_BASE=/serch-demo bun run build   → для GitHub Pages
 *     bun run build                          → для своего домена
 */
const base = process.env.SERCH_BASE || undefined;

export default defineConfig({
  base,
  integrations: [react()],
  // Без явной привязки dev-сервер поднимается только на IPv6-петле (`::1`),
  // и браузер, резолвящий `localhost` в IPv4, получает ERR_CONNECTION_REFUSED.
  // 127.0.0.1 чинит это, не открывая сервер в локальную сеть.
  server: {
    host: '127.0.0.1'
  },
  vite: {

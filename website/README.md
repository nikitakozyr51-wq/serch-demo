...
Scroll reveal and counters live in one place: the module script at the end of `src/layouts/BaseLayout.astro`, with tokens and the `[data-rev]` rules in `src/styles/global.css`. No motion libraries: CSS transitions plus `IntersectionObserver`. Values (0.78s, `cubic-bezier(.22,.61,.24,1)`, 26px, 80/120ms stagger capped at 560ms, threshold 0.01, rootMargin `-4%`) come from `design-research/v2/MOTION-SPEC.md` and were set after a rejected first round; do not retune them by eye.

Markup declares intent, never state:

| Attribute | Meaning |
| --- | --- |
| `data-rev-group` | container whose rendered direct children reveal in sequence |
| `data-rev-step="120"` | stagger step for that group (default 80; 120 for statements and cards) |
| `data-rev-solo` | single block, reveals on its own |
| `data-count-to` | counter target; optional `data-count-from`, `data-count-dur`, `data-count-pow` |

The `data-rev` attribute itself is added by the script, so with JS disabled nothing is hidden. Counters render their final value server-side and are reset to the start value on init. Do not nest one reveal inside another, and do not put `will-change` on unrevealed nodes.

...
Поисковик прямых собственников недвижимости для агентств. Репозиторий собран из
шаблона [di-sukharev/vibe](https://github.com/di-sukharev/vibe), ветка `mobile`
(она включает все поверхности). Ниже — решения проекта, затем документация шаблона.

## Решения проекта

- **Слаг:** `serch`. Он же — scope пакетов (`@serch/*`), имя базы (`serch`, `serch_test`),
  префикс cookie и Compose-проекта. Идентификаторов шаблона (`web-app-demo`,
  `vibecoding-template`) в коде не осталось.
- **Remote:** `origin` — `github.com/nikitakozyr51-wq/serch-demo`, собственный репозиторий
  пользователя. Шаблонный remote не подключался, PR в шаблон не предполагаются.
- **Активные поверхности:** все четыре — `backend`, `webapp`, `website`, `mobile`.
  Отложенных поверхностей нет.
  - `website` (Astro) — публичное и индексируемое: лендинг, тарифы, SEO-страницы.
  - `webapp` (React CSR) — кабинет агентства за логином: объекты, карточки собственников, воронка.
  - `backend` (Bun/Hono/Prisma/PostgreSQL) — API, авторизация, данные.
  - `mobile` (Expo) — мобильный клиент; Expo/EAS и Maestro пока не сконфигурированы.
...
- **Продуктовые исходники** живут в корне и в git не переносились: `PRODUCT.md`,
  `DESIGN.md`, `DEMO-DATA.md`, `PROJECT-PLAN.md`, `cabinet/`, `research/`,
  `design-research/`, Pencil-файл `serch`. HTML-прототип демо лежит в
  `prototype/index.html` и продакшн-кодом не является.

### Правки шаблона под Windows

Установка велась на Windows, и три проверки шаблона там падали не по вине кода:

- `.gitattributes` с `* text=auto eol=lf` и нормализация рабочего дерева в LF —
  тесты шаблона сравниваются с LF-строками, а `core.autocrlf=true` давал CRLF.
  Для репозитория выставлен `core.autocrlf=false`.
- `mobile/tests/typography.test.ts` — пути приводятся к POSIX-разделителям, иначе
  на Windows не срабатывал allowlist и файлы `typography.tsx`/`theme.ts` считались нарушителями.
- `scripts/prepare-do-specs.test.mjs` — проверка режима `0o600` пропускается на
  `win32` (Node там всегда отдаёт `0o666`); на Linux/macOS проверка работает как прежде.
- `scripts/ci-shape.test.mjs` убран из `test:deploy`: он проверяет CI самого шаблона
  (ветки `master`/`mobile`), а рабочая ветка проекта — `main`. Файл оставлен в репозитории.

### Что проверено при установке

Прогнано локально: `architecture:check` (426 файлов), `typecheck` (5 воркспейсов, 0 ошибок),
`test:contracts` (18), `test:backend:unit` (137), `test:webapp` (47), `test:website` (2),
`test:mobile` (190), `test:deploy` (54) — всё зелёное.

**Не проверено:** всё, что требует PostgreSQL — `test:backend:integration`, `e2e:webapp`,
`smoke:backend:docker`, миграции и сид. Docker Desktop на машине не установлен;
до его установки backend локально не поднимается.
